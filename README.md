...
The pipeline expects C3D recordings organized as `Subject/activity/file.c3d`, where activity is one of `walk`, `sit`, `squat`, `stair`, `all`, or `walk+squat+stair`. If no C3D files are present, it creates a deterministic synthetic smoke-test dataset so the repository remains runnable without manual intervention.
